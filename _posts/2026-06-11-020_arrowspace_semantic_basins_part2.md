---
title: "arrowspace for Latent Spaces — part 2"
layout: blog_default
date: 2026-06-11
permalink: /posts/020_arrowspace_semantic_basins_part2/
categories: [arrowspace, vector-search, latent-space, mechanistic-interpretability, python]
excerpt: "Comparative semantic probing: dual-space analysis on transformer weights and embeddings"
---
...
*Notebook:* [`06_B_comparative_semantic_probing.ipynb`](https://github.com/tuned-org-uk/arrowspace-analysis/blob/8db95528af48f7e9b59ebbda4c198b380920ec10/notebooks/06_v1/06_B_comparative_semantic_probing.ipynb)
...
$$E_\text{tok}[\text{token_id}] \in \mathbb{R}^{384}$$
...
Dual space is a fundamental characteristics of ArrowSpace/Graph Wiring, every vector space is leveraged by its geometric space (item-space) and its semantic space (feature-space). The contribution of feature-space has been demonstrated to be relevant as measured by epiplexity in [this paper](https://www.authorea.com/doi/full/10.22541/au.177430060.02394540) and the source of the advanteages provided in search capabilities. 
...
- **$$W_\text{ffn2_read}$$ (dual / readout operator):** the FFN output matrix has shape `(384, 1536)`. For probing, its *transpose* is used — shape `(1536, 384)` — measuring $$\|W_\text{ffn2}^\top x\|_2 / \|W_\text{ffn2}\|_F$$. This answers: *which FFN hidden neurons are most sensitive to this token direction?*
...
$$L_\text{feat} = X X^\top \quad \text{(item-space)} \quad \longleftrightarrow \quad L_\text{item} = X^\top X \quad \text{(feature-space)}$$
...
We have presented here the first simple example of Graph Wiring used as a mechanistic interpretability primitive, treating transformer weight matrices as spectral operators on a semantic *k*NN graph so that ArrowSpace’s Rayleigh-style energy cleanly links embedding geometry, Laplacian smoothness, and layer-wise circuit probes under one unified spectral language.

Graph wiring — the *k*NN topology that ArrowSpace draws on the feature manifold — here becomes a mechanistic interpretability primitive: a Frobenius-normalised projection norm that maps which weight-matrix subspaces encode which semantic fields, layer by layer, in both the primal write direction and its dual readout transpose.

- [Graph Wiring: Eigenstructures for vector datasets and LLM operations](https://www.tuned.org.uk/assets/graph_wiring.pdf)
- [Epiplexity And Graph Wiring: An Empirical Study for the Design of a Generic Algorithm](https://www.tuned.org.uk/graph-wiring)
- [Spectral Indexing using taumode (λτ) with arrowspace](https://www.tuned.org.uk/assets/pathway_for_vectordb_october_2025.pdf)
- [ArrowSpace: Spectral Search For Embeddings and Graph Analysis](https://www.tuned.org.uk/arrowspace-paper)

---

*Notebook:* [`06_B_comparative_semantic_probing.ipynb`](https://github.com/tuned-org-uk/arrowspace-analysis/blob/8db95528af48f7e9b59ebbda4c198b380920ec10/notebooks/06_v1/06_B_comparative_semantic_probing.ipynb)  
*Figures:* [`fig_03a`](https://github.com/tuned-org-uk/arrowspace-analysis/blob/8db95528af48f7e9b59ebbda4c198b380920ec10/notebooks/06_v1/output__06/fig_03a_semantic_subspace_matrix_etok.png), [`fig_03b`](https://github.com/tuned-org-uk/arrowspace-analysis/blob/8db95528af48f7e9b59ebbda4c198b380920ec10/notebooks/06_v1/output__06/fig_03b_semantic_subspace_matrix_pass.png), [`fig_03c`](https://github.com/tuned-org-uk/arrowspace-analysis/blob/8db95528af48f7e9b59ebbda4c198b380920ec10/notebooks/06_v1/output__06/fig_03c_delta_S_dual_space.png)  
