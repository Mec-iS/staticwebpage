...
4. **Boundary tokens and boundary sentences expose the method's limits.** ArrowSpace does not reliably reject polysemous items. Spectral smoothness correlates with semantic purity but is not equivalent to it. Rejecting genuine ambiguity requires a third criterion — possibly cross-layer or cross-model basin agreement.
